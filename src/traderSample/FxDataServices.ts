...
export function useFxData(): StateTradeData {
  const [state, dispatch] = useReducer(reducerTraderApp, initialState);
  useEffect(() => {
    const rowData = createFxMatrixSnapshot();
    dispatch({
      type: TraderAppActionType.FX_DATA_CHANGED,
      payload: rowData,
    });
  }, []);
  useEffect(() => {
...
function applyDeltasToFxData(fxData: any[]) {
  let fxSymbolsToUpdate = _.sampleSize(
    FX_CURRENCY_SYMBOLS,
    FX_CURRENCY_SYMBOLS.length / 5
  );
  let rowsToUpdate = _.sampleSize(fxData, fxData.length / 5);
...
function createFxMatrixSnapshot(): any[] {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    let multiplier = Math.random() * 10 > 5 ? -1 : 1;
    row["pct_net_change"] = multiplier * (random(30, 100) / 100); //(random(30, 100) / 100).toFixed(2));
    rowData.push(row);
  }
  return rowData;
}
...
const fxCurrencySymbol: ColDef[] = FX_CURRENCY_SYMBOLS.map((symbol) => {
  return {
    field: symbol,
    headerName: symbol,
    width: 67,
...
// const SquareRenderer: ICellRendererFunc = (params: any) => {
//   return params.data;
// };

export const FX_DELTA_HEADERS: ColDef[] = [
  {
    field: "symbol",
    headerName: "Symbol",
    width: 80,
